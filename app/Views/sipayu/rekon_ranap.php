...
<script>
    ...
    $(document).ready(function() {
        var table = $('#getjasarajal').DataTable({
            ...
            "order": true,
            "ajax": {
                "url": "<?= base_url('sipayu/getjasaranap') ?>",
                "type": "GET",
                "data": function(d) {
            ...
            "columns": [{
                ...
                },
                {
                    ...
                    "searchable": false,
                    "render": function(data, type, row) {
                        let jasaPemeriksaan = parseFloat(row.JasaMedisRvu) || 0;
                        let jasaTindakan = parseFloat(row.JasaMedisTindakanRvu) || 0;
                        let total = jasaPemeriksaan + jasaTindakan;
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return '<button class="btn btn-outline-success btn-xsx" data-bs-toggle="tooltip" data-bs-placement="left" title="Lihat Detail" type="button" onclick="window.open(\'<?= base_url('sipayu/detail_ri') ?>?idx=' + row.IdRegisterKunjungan + '\', \'_blank\')" style="font-size: 10px;padding: 0px 4px"> <i class="bi bi-eye-fill"></i> </button>';
                    }
                }
            ],
            ...
            "footerCallback": function(row, data, start, end, display) {
                ...
                let totalmedis = data.reduce((total, row) => {
                    let jasaPemeriksaan = intVal(row.JasaMedisRvu);
                    let jasaTindakan = intVal(row.JasaMedisTindakanRvu);
                    return total + (jasaPemeriksaan + jasaTindakan);
                }, 0);
