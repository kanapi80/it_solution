                    ...
                    <div class="card-body mt-4">
                        <div class="col-lg-12">
                            <div class="row g-2 align-items-center mb-2">
                                ...
                                <div class="col-auto">
                                    <select name="anastesi" id="anastesi" class="form-control form-control-sm">
                                        <option value="">-</option>
                                        <option value="Tindakan Anastesi">Anastesi</option>
                                    </select>
                                </div>
                        ...
                        <table class="table table-striped w-100 table-sm table-borderless" id="getjasarajal" style="font-size: 9px;">
                            <thead>
                                <tr>
                                    ...
                                    <th class="bg-success text-white text-start" width="4%">NOMR</th>
                                    <th class="bg-success text-white text-start" width="15%">NAMA</th>
                                    <th class="bg-success text-white text-start" width="27%">NAMA TINDAKAN</th>
                                    <th class="bg-success text-white text-start" width="20%">DOKTER</th>
                                    <th class="bg-success text-white text-start" width="5%">TANGGAL</th>
                                    <th class="bg-success text-white text-start" width="8%">RUANGAN</th>
                                    <th class="bg-success text-white text-center" width="5%">TARIF</th>
...
<script>
    ...
    $(document).ready(function() {
        var table = $('#getjasarajal').DataTable({
            ...
            "order": true,
            "ajax": {
                "url": "<?= base_url('sipayu/getjasaoperasi') ?>",
                "type": "GET",
                "data": function(d) {
            ...
            "columns": [{
                ...
                {
                    "data": null, // Data diambil dari dua kolom, jadi pakai null
                    "className": "text-end",
                    "searchable": false,
                    "render": function(data, type, row) {
                        let jasaDokter = parseFloat(row.JasaMedisDokterRvu) || 0;
                        let jasaAnestesi = parseFloat(row.JasaMedisDokterAnestesiRvu) || 0;
                        let total = jasaDokter + jasaAnestesi;
                ...
                },
                {
                    ...
                    "searchable": false,
                    "render": function(data, type, row) {
                        let jasaDokter = parseFloat(row.JasaMedisPerawatBedahRvu) || 0;
                        let jasaAnestesi = parseFloat(row.JasaMedisPenataAnestesiRvu) || 0;
                        let total = jasaDokter + jasaAnestesi;
            ...
            "footerCallback": function(row, data, start, end, display) {
                ...
                let intVal = function(i) {
                    if (typeof i === 'string') {
                        i = i.replace(/[^\d.-]/g, ''); // Hapus semua karakter kecuali angka dan titik
                    }
                    let parsed = parseFloat(i);
                    return isNaN(parsed) ? 0 : parsed;
                };

                // Hitung total dari kolom perhitungan: JasaMedisDokterRvu + JasaMedisDokterAnestesiRvu
                let totalMedis = data.reduce((total, row) => {
                    let jasaDokter = intVal(row.JasaMedisDokterRvu);
                    let jasaAnestesi = intVal(row.JasaMedisDokterAnestesiRvu);
                    return total + (jasaDokter + jasaAnestesi);
                }, 0);

                let totalParamedis = data.reduce((total, row) => {
                    let jasaPerawat = intVal(row.JasaMedisPerawatBedahRvu);
                    let jasaPenataAnestesi = intVal(row.JasaMedisPenataAnestesiRvu);
                    return total + (jasaPerawat + jasaPenataAnestesi);
                }, 0);

                let totalKebersamaan = api.column(10, {
                        page: 'current'
                    }).data()
                    .reduce((a, b) => intVal(a) + intVal(b), 0);

                // Format angka menjadi dua digit desimal
